$(function () {
    ...
    if (typeof should_crop !== 'undefined') {
        return
    }

    var $modal = $('#modal');
    let pic_id = Math.random()
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (file.type.match('image')) {
                ...
                picReader.addEventListener("load", function (event) {
                    ...
                    div.innerHTML = "<img id='" + pic_id + "_img' src='" + picFile.result + "'" +
                        "title='" + picFile.name + "'/><input id='" + pic_id + "_in' type=\"hidden\" name=\"imagens[]\" value=\"" + picFile.result + "\" ><div  class='post-thumb'><div class='inner-post-thumb'><a href='javascript:void(0);' data-id='" + event.target.fileName + "' class='remove-pic'><i class='fa fa-times' aria-hidden='true'></i></a><div></div>";
            ...
            if (file.type.match('image')) {
                ...
                picReader.addEventListener("load", function (event) {
                    ...
                    if ($('ul.clearfix > li').length > 1) {
                        var removeItem = document.getElementById("imgtmp").getAttribute("value");
                        var $promise = $.ajax({
            ...
            if (file.type.match('image')) {
                ...
                picReader.addEventListener("load", function (event) {
                    ...
                    if ($('ul.clearfix-mobile > li').length > 1) {
                        var removeItem = document.getElementById("imgtmp-mobile").getAttribute("value");
                        var $promise = $.ajax({
                            url: '/api/backoffice/imageDelete/' + removeItem,
                            type: 'GET',
                            success: function (response, status) {
